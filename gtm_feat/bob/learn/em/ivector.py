...
class IVectorStats:
    ...
    **Attributes:**
    ...
    def __init__(self, dim_c, dim_d, dim_t):
        ...
        self.nij_sigma_wij2 = np.zeros(shape=(self.dim_c, self.dim_t, self.dim_t), dtype=float)
        # fnorm sigma wij: shape = (c,d,t)
        self.fnorm_sigma_wij = np.zeros(shape=(self.dim_c, self.dim_d, self.dim_t), dtype=float)
...
def compute_id_tt_sigma_inv_t(stats: GMMStats, T: np.ndarray, sigma: np.ndarray) -> np.ndarray:
    dim_t = T.shape[-1]
    tct_sigmac_inv_tc = compute_tct_sigmac_inv_tc(T, sigma)

    output = np.eye(dim_t, dim_t) + np.einsum("c,ctu->tu", stats.n, tct_sigmac_inv_tc)
...
def e_step(machine: "IVectorMachine", data: List[GMMStats]) -> IVectorStats:
    ...
    for sample in data:
        ...
        sigma_w_ij2 = I_TtSigmaInvNT_inv + np.outer(sigma_w_ij, sigma_w_ij)  # shape: (t,t)
...
def m_step(machine: "IVectorMachine", stats: IVectorStats) -> "IVectorMachine":
    ...
    if any(mask := A.any(axis=(-2, -1))):  # Prevents solving with 0 matrices
        X[mask] = [np.linalg.solve(A[c], B[c]) for c in range(len(mask)) if A[c].any()]

    # Update the machine
    machine.T = X.transpose((0, 2, 1))

    if machine.update_sigma:
        fnorm_sigma_wij_tt = np.diagonal(stats.fnorm_sigma_wij @ X, axis1=-2, axis2=-1)
        machine.sigma = (stats.snormij - fnorm_sigma_wij_tt) / stats.nij[:, None]
        machine.sigma[machine.sigma < machine.variance_floor] = machine.variance_floor
    ...
    def __init__(
        ...
        **Parameters**
        ...
        if self.convergence_threshold:
            logger.info("The convergence threshold is ignored by IVectorMachine.")

        self.output_folder = output_folder or MODELS_PRETRAINED_DIR / "i_vector"

    def fit(self, X: Union[List[np.ndarray], dask.bag.Bag], y=None) -> "IVectorMachine":
        ...
        step = 0
        for step in tqdm.tqdm(range(self.max_iterations)):
            logger.info(
            ...
            if chunky:
                ...
                while (length := len(stats)) > 1:
                    last = stats[-1]
                    stats = [
                        dask.delayed(operator.add)(stats[i], stats[length // 2 + i])
                ...
                new_machine = dask.compute(dask.delayed(m_step)(self, stats_sum))[0]
                for attr in ["T", "sigma"]:
            ...
            # Save a checkpoint
            checkpoint_path = self.output_folder / "ivector_machine-checkpoint.pt"
    ...
    def project(self, stats: GMMStats) -> np.ndarray:
        ...
        **Returns:**
        ...
        return np.linalg.solve(
            ...
            compute_tt_sigma_inv_fnorm(self.ubm.means, stats, self.T, self.sigma),
        )
