...
def logaddexp_reduce(array, axis=0, keepdims=False):
    return np.logaddexp.reduce(array, axis=axis, keepdims=keepdims, initial=-np.inf)
...
def log_weighted_likelihood(data, machine):
    ...
    for i in range(n_gaussians):
        temp = np.sum((data - machine.means[i]) ** 2 / machine.variances[i], axis=-1)
        z.append(temp)
    z = np.vstack(z)
...
def e_step(data, machine):
    ...
    log_weighted_likelihoods = log_weighted_likelihood(data=data, machine=machine)
...
class GMMStats:
    ...
    def __init__(self, n_gaussians: int, n_features: int, like=None, **kwargs) -> None:
        ...
        self.sum_px = np.zeros(shape=(self.n_gaussians, self.n_features), dtype=float, **kw)
        self.sum_pxx = np.zeros(shape=(self.n_gaussians, self.n_features), dtype=float, **kw)
...
class GMMMachine(BaseEstimator):
    ...
    * Maximum a Posteriori (:ref:`MAP <map>`)
    ...
    @weights.setter
    def weights(self, weights: "np.ndarray[('n_gaussians',), float]"):  # noqa: F821  # noqa: F821
    ...
    def initialize_gaussians(
        ...
        if self.trainer == "map":
            ...
            logger.debug("Estimating the variance and weights of each gaussian from kmeans.")
            (
                self.variances,
                self.weights,
    ...
    def fit(self, X, y=None):
        ...
        while self.max_fitting_steps is None or step < self.max_fitting_steps:
            ...
            if input_is_dask:
                ...
                new_machine, average_output = dask.compute(dask.delayed(m_step)(stats, self))[0]
            ...
            logger.info(f"log likelihood = {average_output}")
            if step > 1:
            ...
            # Save checkpoint
            checkpoint_path = self.output_folder / "ubm_checkpoint.pt"
    ...
    def stats_per_sample(self, X):
        return [e_step(data=xx, machine=self) for xx in tqdm.tqdm(X)]
