...
class FactorAnalysisBase(BaseEstimator):
    ...
    def _compute_fn_x_ih(self, x_i, latent_z_i=None, latent_y_i=None):
        ...
        if latent_z_i is None:
            ...
            fn_x_ih = f_i.flatten() - n_ic * (self.mean_supervector + self._D * latent_z_i)
    ...
    def compute_latent_x(self, *, X, y, n_classes, UProd, latent_y=None, latent_z=None):
        """
    ...
    def _compute_latent_x_per_class(self, *, X_i, UProd, UTinvSigma, latent_y_i, latent_z_i):
        # For each sample
        latent_x_i = []
        for x_i in X_i:
            id_plus_prod_ih = self._compute_id_plus_u_prod_ih(x_i, UProd)

            fn_x_ih = self._compute_fn_x_ih(x_i, latent_z_i=latent_z_i, latent_y_i=latent_y_i)
    ...
    def update_U(self, acc_U_A1, acc_U_A2):
        ...
        U_c = acc_U_A2.reshape(self.ubm.n_gaussians, self.feature_dimension, self.r_U) @ inv_A1
        self._U = U_c.reshape(self.ubm.n_gaussians * self.feature_dimension, self.r_U)
        return self._U
    ...
    def _compute_uprod(self):
        ...
        Uc = self._U.reshape((self.ubm.n_gaussians, self.feature_dimension, self.r_U))
    ...
    def compute_accumulators_U(self, X, y, UProd, latent_x, latent_y, latent_z):
        ...
        # U accumulators
        acc_U_A1 = np.zeros((self.ubm.n_gaussians, self.r_U, self.r_U), like=X[0].n)
        acc_U_A2 = np.zeros((self.supervector_dimension, self.r_U), like=X[0].n)

        # Loops over all people
        for y_i in set(y):
            # For each session
            for session_index, x_i in enumerate(self._get_statistics_by_class_id(X, y, y_i)):
                id_plus_prod_ih = self._compute_id_plus_u_prod_ih(x_i, UProd)
                latent_z_i = latent_z[y_i] if latent_z is not None else None
                latent_y_i = latent_y[y_i] if latent_y is not None else None
                fn_x_ih = self._compute_fn_x_ih(x_i, latent_y_i=latent_y_i, latent_z_i=latent_z_i)

                latent_x_i = latent_x[y_i][:, session_index]
                id_plus_prod_ih += latent_x_i[:, np.newaxis] @ latent_x_i[:, np.newaxis].T

                acc_U_A1 += mult_along_axis(
                    id_plus_prod_ih[np.newaxis].repeat(self.ubm.n_gaussians, axis=0),
                    x_i.n,
                    axis=0,
                )
    ...
    def update_z(self, X, y, latent_x, latent_y, latent_z, n_acc, f_acc):
        ...
        # for each class
        for y_i in set(y):
            id_plus_d_prod = self._compute_id_plus_d_prod_i(dt_inv_sigma_d, n_acc[y_i])
            X_i = self._get_statistics_by_class_id(X, y, y_i)
            latent_x_i = latent_x[y_i]

            latent_y_i = latent_y[y_i] if latent_y is not None else None

            fn_z_i = self._compute_fn_z_i(X_i, latent_x_i, latent_y_i, n_acc[y_i], f_acc[y_i])
    ...
    def compute_accumulators_D(self, X, y, latent_x, latent_y, latent_z, n_acc, f_acc):
        """
        ...
        for y_i in set(y):
            ...
            acc_D_A1 += (id_plus_d_prod + latent_z[y_i] * latent_z[y_i]) * tmp_CD
    ...
    def initialize_XYZ(self, n_samples_per_class, like=None):
        ...
        latent_y = np.zeros((n_classes, self.r_V), **kw) if self.r_V and self.r_V > 0 else None
    ...
    def _compute_vprod(self):
        ...
        Vc = self._V.reshape((self.ubm.n_gaussians, self.feature_dimension, self.r_V))
    ...
    def compute_accumulators_V(self, X, y, VProd, n_acc, f_acc, latent_x, latent_y, latent_z):
        """
        ...
        # U accumulators
        acc_V_A1 = np.zeros((self.ubm.n_gaussians, self.r_V, self.r_V), like=X[0].n)
        ...
        for i in set(y):
            ...
            id_plus_prod_v_i += latent_y_i[:, np.newaxis] @ latent_y_i[:, np.newaxis].T

            acc_V_A1 += mult_along_axis(
                id_plus_prod_v_i[np.newaxis].repeat(self.ubm.n_gaussians, axis=0),
                n_acc_i,
                axis=0,
            )
...
class ISVMachine(FactorAnalysisBase):
    ...
    def e_step(self, X, y, n_samples_per_class, n_acc, f_acc):
        ...
        _, _, latent_z = self.initialize_XYZ(n_samples_per_class=n_samples_per_class, like=X[0].n)
        latent_y = None
        ...
        acc_U_A1, acc_U_A2 = self.compute_accumulators_U(X, y, UProd, latent_x, latent_y, latent_z)
...
class JFAMachine(FactorAnalysisBase):
    ...
    def m_step_v(self, acc_V_A1_acc_V_A2_list):
        ...
        V_c = acc_V_A2.reshape((self.ubm.n_gaussians, self.feature_dimension, self.r_V)) @ inv_A1
        self._V = V_c.reshape((self.ubm.n_gaussians * self.feature_dimension, self.r_V))
        return self._V
    ...
    def e_step_u(self, X, y, n_samples_per_class, latent_y):
        ...
        # self.initialize_XYZ(y)
        UProd = self._compute_uprod()
        latent_x, _, latent_z = self.initialize_XYZ(n_samples_per_class=n_samples_per_class)
    ...
    def finalize_u(
        ...
        UProd = self._compute_uprod()
        latent_x, _, _ = self.initialize_XYZ(n_samples_per_class=n_samples_per_class)
    ...
    def e_step_d(self, X, y, n_samples_per_class, latent_x, latent_y, n_acc, f_acc):
        """
        ISV E-step for the U matrix.
        ...
        _, _, latent_z = self.initialize_XYZ(n_samples_per_class=n_samples_per_class)
    ...
    def enroll(self, X):
        ...
        UProd = self._compute_uprod()
        VProd = self._compute_vprod()
        latent_x, latent_y, latent_z = self.initialize_XYZ(n_samples_per_class=[len(X)])
