...
def e_step(data, means):
    ...
    zeroeth_order_statistics = np.bincount(closest_k_indices, minlength=n_clusters)
...
def accumulate_indices_means_vars(data, means):
    ...
    for i in range(n_clusters):
        variances_sum[i] = np.sum(data[closest_centroid_indices == i] ** 2, axis=0)
...
class KMeansMachine(BaseEstimator):
    ...
    def get_variances_and_weights_for_each_cluster(self, data: np.ndarray):
        ...
        if input_is_dask:
            stats = [
                dask.delayed(accumulate_indices_means_vars)(xx, means=self.centroids_)
                for xx in data
            ]
            variances, weights = dask.compute(dask.delayed(reduce_indices_means_vars)(stats))[0]
        else:
            # Accumulate
    ...
    def initialize(self, data: np.ndarray):
        ...
        logger.info(f"Initializing k-means means with '{self.init_method}'.")
        ...
        logger.info("End of k-means initialization")
    ...
    def fit(self, X, y=None):
        ...
        while self.max_iter is None or step < self.max_iter:
            ...
            if input_is_dask:
                stats = [dask.delayed(e_step)(xx, means=self.centroids_) for xx in X]
                ...
                self.centroids_, self.average_min_distance = m_step(stats, n_samples)

            distance = self.average_min_distance

            logger.debug(f"Average minimal squared Euclidean distance = {distance}")
    ...
    def predict(self, X):
        ...
        """
        return get_closest_centroid_index(get_centroids_distance(X, self.centroids_))
